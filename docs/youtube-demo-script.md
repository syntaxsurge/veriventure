...
## Segment 1 – Home Hero Intro

1. **On-screen:** Time `00:00–00:20`. Start recording with the homepage (`/`) visible and no prior navigation. Slowly move the mouse across the hero title and subtitle, but don’t click anything yet.
   - **Voice-over 1:** “Welcome to VeriVenture, the entrepreneur OS that helps you get paid faster, prove your traction with real proofs, and share a verification page that buyers and investors can trust.”

## Segment 2 – Connect Wallet

1. **On-screen:** Time `00:20–00:45`. Stay on the homepage (`/`). Move the cursor to the `Connect Wallet` button in the header or hero, click it, approve the connection in the Polkadot/Talisman extension, and wait until the UI clearly shows you as connected.
   - **Voice-over 2:** “Everything starts from the homepage. I connect my wallet to sign in—no passwords, just my wallet identity—so every invoice, credential, and proof is tied back to a single founder profile.”

## Segment 3 – Passport & Handle
...
1. **On-screen:** Time `01:15–01:45`. Navigate to `/dashboard`. Point to the wallet chip or profile section showing the connected address, then point to the quick-start tiles or checklist (for example actions like Create Invoice, Mint Credential, or Generate Deck).
   - **Voice-over 4:** “On the dashboard I get a quick snapshot of my account: which wallet I’m using and the main actions I can take. The checklist is designed for founders—create or review a credential, send an invoice, or launch the AI tools to generate pitch decks and business plans.”

## Segment 5 – Add Credential
...
3. **On-screen:** Then open the Resume page at `/ai-assistant/resume`, and show a generated summary based on your founder profile.
   - **Voice-over 7:** “The same AI hub also gives me a business plan writer and a resume builder. I can generate narrative sections for my plan and a founder CV without starting from a blank page, and all of these documents will be available later in the Documents area.”

## Segment 8 – Create Invoice
...
2. **On-screen:** On the public invoice view, click `Pay invoice` and complete the payment using a test wallet, or, if payment rails are mocked in your environment, click a `Mark as Paid` button that simulates payment.
   - **Voice-over 9:** “This is what my client sees when they open the invoice link—a clear summary and a single action to pay. They don’t have to create an account; they can simply pay with their wallet or through whatever payment option I’ve configured, and the status will update on my side.”

## Segment 10 – Publish Payment Proof
...
2. **On-screen:** Show the newly created proof entry for the paid invoice with its UAL and chain transaction link. Click a toggle such as `Feature on Profile` for that payment proof, and feature one credential proof as well if your UI supports it.
   - **Voice-over 11:** “All of my proofs live in the Proofs and Audit Log area. Here I can see the payment proof I just published, along with its DKG UAL and chain transaction link. I can choose which items I want to feature on my public profile—for example, this settled invoice and my key milestone credentials—so my verify page shows the highlights instead of everything.”

## Segment 12 – Documents Vault
...
2. **On-screen:** Highlight the pitch deck and business plan entries that were generated earlier. Open the pitch deck entry and show the `Preview` or `Download` buttons.
   - **Voice-over 12:** “The Documents page is my archive of AI-generated artefacts—pitch decks, business plans, resumes, and other content. I can re-open any document to tweak it, or download it as a PDF or PPTX for email or data room uploads.”

## Segment 13 – Claim Checker / Truth Alignment
...
1. **On-screen:** Time `07:45–08:10`. Navigate back to the homepage (`/`). Slowly scroll just enough to reveal the hero title and one or two key feature cards again.
   - **Voice-over 15:** “That’s how VeriVenture works end-to-end: create invoices any client can pay, generate investor-ready documents with AI, mint and publish proofs only when they add value, and share a single verify link that turns your story into something people can click to trust.”
